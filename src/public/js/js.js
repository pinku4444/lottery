...
function myCreateFunction() {
  ...
  if (selectType == 'self' && (gMyTableOne.length < 5 || gMyTableTwo.length < 1))
    return toastr.error("Minium 5 numbers and 1 SE required");
  if (selectType === 'self') {
    noOfRows = 1;
  }
...
function saveUserData(Idata) {
  ...
  $.ajax({
    ...
    dataType: 'json',
    data: JSON.stringify({ data: Idata }),
...
function showResult(mainData, genNumbers, currentDrawnumber) {
  ...
  if (isPremium()) {
    ...
    $.ajax({
      ...
      dataType: 'json',
      data: JSON.stringify({ data: dataTobeUpdate }),
